...
before_action :set_user, except: [:my_photos, :subscribes_list, :friends_photos]
    ...
    if current_user.id == @user.id
     redirect_to profile_path(@user), notice: "Вы не можете подписаться на самого себя"
       ...
       redirect_to profile_path(@user), notice: "Вы уже подписаны на данного пользователя"
        ...
        @subscription.freind_id = @user.id
        @subscription.save
  ...
  def unsubscribe
     if current_user.id == @user.id
       ...
       if current_user.subscriptions.exists?(freind_id: @user.id)
        @subscription = current_user.subscriptions.find_by_freind_id(@user.id)
  ...
  def subscribes_list
     @friends = User.where(id: current_user.subscriptions.pluck(:freind_id))
  end

  def friends_photos
     @photos = Photo.where(user_id: current_user.subscriptions.pluck(:freind_id)).order('created_at DESC')
  end
